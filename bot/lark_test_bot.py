...
- 涉及具体掉线电机时，先按 SN 前缀确定机器形态（DACH=双臂用 2.1 表 / SF=双足用 2.2 表 / WF=轮足用 2.3 表；HU_D04=人形机器），再用区块内对应「网络拓扑与从站号对照表」把 motor 号换算成 slave 号，并按串行拓扑指出应重点排查的 M-1↔M 链路（若 M-1 是 CU1128 分支器——双臂 slave1/slave10、双足与轮足 slave1/slave7——则优先怀疑分支器及其上联链路）。若参考文件中暂无该形态（如 HU_D04 人形）的拓扑对照表，则如实说明“暂无该形态拓扑表，无法把 motor 精确换算为 slave”，只依据日志中直接出现的 slave/端口证据定位，禁止套用其他形态的对照表。
