...
class HumanoidWorkflowTests(unittest.TestCase):
    ...
    def test_diagnostic_value_change_point_compression(self):
        ...
        for i in range(6):
            lines.append(
                f"2026-08-08 11:00:0{i}.000 I/monitor(m)(2/2): DiagnosticValue - (name: imu, part: , level: OK, code: 0, message: )"
            )
        lines.append(
            "2026-08-08 11:00:00.500 I/monitor(m)(2/2): DiagnosticValue - (name: ability_running, part: , level: OK, code: 0, message: stand)"
        )
        lines.append(
            "2026-08-08 11:00:05.500 I/monitor(m)(2/2): DiagnosticValue - (name: ability_running, part: , level: OK, code: 0, message: walk)"
        )
        with tempfile.TemporaryDirectory() as tmpdir:
            path = Path(tmpdir) / "humanoid.log.active"
            path.write_text("\n".join(lines), encoding="utf-8")
